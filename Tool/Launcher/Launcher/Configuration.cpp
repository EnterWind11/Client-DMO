...
void sConfiguration::_GetMainUrlInfo( Json::Value const& root, std::map<CString,sMainUrlInfo> & sData )
{
	...
	for( ; it != jUrlInfo.end(); ++it )
	{
		...
		addInfo.m_ClientIP = (*it).get("ClientIP", "").asWString().c_str();
		addInfo.m_ClientPort = (*it).get("ClientPort", "").asWString().c_str();
...
CString sConfiguration::GetGameIp(CString const& userType) const
{
	std::map<CString, sMainUrlInfo>::const_iterator it = m_mainUrlInfo.find(userType);
	if (it == m_mainUrlInfo.end())
		return _T("");
	return it->second.m_ClientIP;
}

CString sConfiguration::GetGameIpPort(CString const& userType) const
{
	std::map<CString, sMainUrlInfo>::const_iterator it = m_mainUrlInfo.find(userType);
	if (it == m_mainUrlInfo.end())
		return _T("");
	return it->second.m_ClientPort;
}
//////////////////////////////////////////////////////////////////////////
